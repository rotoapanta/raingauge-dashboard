  ...
  return (
    <div className="p-4 space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div className={`text-sm font-semibold px-3 py-1 rounded-full ${isOnline ? "bg-green-600 text-white" : "bg-red-600 text-white"}`}>
          {isOnline ? (
            <span className="flex items-center gap-2"><Wifi size={14} /> Online</span>
          ) : (
            <span className="flex items-center gap-2"><WifiOff size={14} /> Offline</span>
          )}
        </div>
        <div className="text-sm text-gray-300">
          Último ping: {lastPing}
        </div>
      </div>
      ...
      {status ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          <Card icon={<Thermometer />} title="CPU Temp" value={status.temp !== null ? `${status.temp} °C` : "N/A"} />
          <Card icon={<Terminal />} title="Hostname" value={status.hostname} />
          <Card icon={<Cloud />} title="IP Address" value={status.ip} />
          <Card
          ...
          <tbody>
            {logs.slice().reverse().map((log, i) => (
              <tr key={i} className="border-t border-gray-700">
                ...
                <td className={`px-4 py-2 font-bold ${log.status === "ONLINE" ? "text-green-400" : "text-red-400"}`}>
                  {log.status}
                </td>
              </tr>
            ))}
...
function BatteryIcon({ status }: { status: string }) {
  const color =
    status === "CRITICAL"
      ...
      ? "text-yellow-400"
      : "text-green-400";
  return <div className={color}><Battery /></div>;
}
